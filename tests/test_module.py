...
class TestModule(unittest.TestCase):
        ...
        try:
            result = acme_tiny.main(
                [
                    ...
                    self.KEYS["account_key"].name,
                    "--csr",
                    self.KEYS["nonexistent_csr"].name,
                    "--acme-dir",
                    self.tempdir,
                    "--directory-url",
                    self.DIR_URL,
    ...
    @unittest.skipIf(
        USE_STAGING,
        "only checked on pebble server since staging can't have nonce retries set",
    )
    ...
    def test_pebble_doesnt_support_cn_domains(self):
        ...
        self.assertIsInstance(result, ValueError)
        self.assertIn(
            "Order includes different number of DNSnames identifiers than CSR specifies",
            result.args[0],
        )
    ...
    @unittest.skipIf(
        (not USE_STAGING),
        "only checked on staging since pebble doesn't support CN names",
    )
    ...
    @unittest.skipIf(
        (not USE_STAGING),
        "only checked on staging since pebble doesn't check for weak keys",
    )
